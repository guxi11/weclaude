...
flowchart LR
    ...
    tagged --> exist
    exist -- 否 --> spawn["auto-spawn tmux pane<br/>+ claude 会话 (首条消息即 prompt)"]
...
- **cwd 是聊天级的**：同一聊天所有 tagged / 默认会话共用一个 cwd，`/new #foo` 在聊天绑定的 cwd 下起 pane，`cd` MCP 写入的 pendingCwd 也挂聊天层。
- **命名聊天打开跨群地址空间**：`/name` 之后 `fix`（本聊天优先，回退全局唯一）、`daily#fix`（指定聊天的 tag）、`daily#`（默认会话）、`chat:wr…#fix`（全量 key）四级都可寻址，供 `send_peer` / `new_claude_session` 等 MCP 工具使用。未命名聊天不可寻址也不可被建入——刻意的。
